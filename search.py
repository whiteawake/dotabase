    ...
    <body style="margin: 0px">
        <nav>
            <ul>
                ...
                <li class="current"><a class="menu" href="query.py#/heroes">Letter Query</a></li>
        ...
        <article id="boxedheroes2" style="margin-left: 330px;">
        <div id="querytable">
            <table data-table-sortable="yes" style="margin-top: -948px;">
    """)
...
cursor.execute("""SELECT COUNT(hero_name) FROM Hero
			    WHERE hero_name LIKE :start_letter""", values)
...
if(len(records) == 0):
    ...
    print('<font color="white">There are ' + str(records[0][0]) + ' heroes with a name starting with \'' + start_letter + '\'</font>')
print('<br /><br />')

cursor.execute('''SELECT hero_name, trait, speed, max_damage
		    FROM Hero
		    WHERE hero_name LIKE :start_letter''', values)

fields = ["Heroes (" + str(records[0][0]) + ")", "Primary Trait", "Speed", "Damage"]
records = cursor.fetchall()
